...
export function registerFileIpc() {
    ipcMain.handle('download-file', async (event, username, filename) => {
        ...
        try {
            ...
            const response = await axios({
                method: 'GET',
                url: `${getAPIBase()}/user/download/${encodeURIComponent(filename)}`,
                responseType: 'stream',
    ...
    ipcMain.handle('upload-file', async (event, username, filename, fileData) => {
        try {
            const response = await axios.post(`${getAPIBase()}/user/upload?username=${username}&filename=${filename}`, fileData, { headers: { 'Content-Type': 'application/octet-stream' } })
    ...
    ipcMain.handle('delete-file', async (event, username, filename) => {
        try {
            const response = await axios.delete(`${getAPIBase()}/user/file/${username}/${filename}`)
            return response.data
        } catch (error) { return { success: false, message: error.message } }
    })
    ...
    ipcMain.handle('get-user-files', async (event, username) => {
        try {
            const response = await axios.get(`${getAPIBase()}/user/files/${username}`)
            return response.data
        } catch (error) {
